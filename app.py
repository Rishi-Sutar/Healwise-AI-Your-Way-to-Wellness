...
def main():
    try:
        ...
        patient_symptoms = st.text_input('Symptoms','Symptoms')

        # Split the user's input into a list of symptoms (assuming they are comma-separated)
        user_symptoms = [s.strip() for s in patient_symptoms.split(',')]
        ...
        if st.button("Predict"):
            ...
            if predicted_disease:
                ...
                st.write("Precaution: ",'\n'.join([f"- {item}" for item in pre[0]]))
                st.write("Medicines: ",med[0])
                st.write("Diet: ",die[0])
